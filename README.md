...
<div><br>Note that I have only tweeted 3 times ever, so that is why only 3 tweets are shown.</div>
...
<div><br>Note that running `node liri.js spotify-this-song "My Name Is Jonas"` will produce the same result.</div>
...
<div><br>Note that in this case, the `log.txt` file contians the command to Spotify the song "I Want It That Way". The `log.txt` file can be changed to perform any of the 3 types of commands listed above.</div>
